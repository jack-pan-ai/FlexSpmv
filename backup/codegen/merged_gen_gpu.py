...
def generate_cuda_code_from_graph(traced_model):
    ...
    for out in outputs:
        ...
        if 'reducer' in str(out['target']):
            ...
            output_agent_tenosrs_code.append(
                f"  typedef TensorKey<OffsetT, ValueT, {_dim}> TensorKeyOutput_{out_name}_T; \n")
            output_agent_tenosrs_code.append(
                f"  typedef Tensor<ValueT, {_dim}> TensorOutput_{out_name}_T; \n")
            output_agent_tenosrs_code.append(
                f"  // Reduce-value-by-segment scan operator \n")
            output_agent_tenosrs_code.append(
                f"  typedef ReduceTensorByKeyOp<TensorKeyOutput_{out_name}_T> ReduceBySegmentOp_{out_name}_T; \n")
            ...
            output_agent_SMEM_code.append(
                f"               SmemReuseReducer<{_dim}, BlockScan_{out_name}_T> smem_{out_name}; \n")
    ...
    print("Generted kernel operations:")
    for op in map_code:
        print("Map code: ", op)
