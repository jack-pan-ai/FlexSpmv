...
def system_test(
    ...
    print(
        f"Running system test with num_nnz={num_nnz}, num_cols={num_cols}, num_rows={num_rows}")
    ...
    output_y_map_2 = torch.zeros(
        num_nnz, 2, 3, device=device, dtype=dtype)  # (N, D)
    ...
    output_y_reducer_2 = torch.zeros(
        num_rows, 2, 3, device=device, dtype=dtype)  # (M, D)
    row_end_offset = torch.sort(
        torch.randint(
            0,
            num_nnz + 1,
            (num_rows - 1,
            ...
            device=device,
            dtype=torch.int64))[0]
    row_end_offset = torch.cat(
        [torch.zeros(1, device=device, dtype=torch.int64), row_end_offset])
    row_end_offset = torch.cat([row_end_offset, torch.tensor(
        [num_nnz], device=device, dtype=torch.int64)])  # (M + 1)
    # print(f"row_end_offset: {row_end_offset}")
    # Generate col_indices_1 and col_indices_2 randomly (can be the same or
    # different)
    ...
    if device == "cuda":
        ...
        forward_time_ms = start_event.elapsed_time(
            end_event)  # Returns time in milliseconds
    ...
    # print(f"Throughput: \
    #         {num_nnz / avg_time_per_iter * 1000:.0f} elements/second")
