...
bool VerifyOmpMergeSystem(int seed, int num_rows, int num_cols, int nnz,
...
#if USE_AGGREGATORS
  std::vector<ValueT> out_agg1(ne1_dim);
  std::vector<ValueT> out_agg2(ne2_dim);
#endif
  ...
  for (size_t i = 0; i < out_red1.size() && ok; ++i)
    if (!almost_equal(out_red1[i], out_red1_ref[i])) {
      ok = false;
      if (verbose)
        std::cout << "red1 mismatch at " << i << " ref: " << out_red1_ref[i]
                  << " computed: " << out_red1[i] << "\n";
      break;
    }
  for (size_t i = 0; i < out_red2.size() && ok; ++i)
    if (!almost_equal(out_red2[i], out_red2_ref[i])) {
      ok = false;
      if (verbose)
        std::cout << "red2 mismatch at " << i << " ref: " << out_red2_ref[i]
                  << " computed: " << out_red2[i] << "\n";
      break;
    }
#endif
#if USE_AGGREGATORS
  for (size_t i = 0; i < out_agg1.size() && ok; ++i)
    if (!almost_equal(out_agg1[i], out_agg1_ref[i])) {
      ok = false;
      if (verbose)
        std::cout << "agg1 mismatch at " << i << " ref: " << out_agg1_ref[i]
                  << " computed: " << out_agg1[i] << "\n";
      break;
    }
  for (size_t i = 0; i < out_agg2.size() && ok; ++i)
    if (!almost_equal(out_agg2[i], out_agg2_ref[i])) {
      ok = false;
      if (verbose)
        std::cout << "agg2 mismatch at " << i << " ref: " << out_agg2_ref[i]
                  << " computed: " << out_agg2[i] << "\n";
      break;
    }
#endif
