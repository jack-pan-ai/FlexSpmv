...
def declarations_gen(inputs, outputs, selector_register, reducer_operations, aggregator_operations):
    #  -------------------------------------------------------------
    ...
    for inp in inputs:
        if inp['dtype'] == 'int':
            # column indices
            input_parameters_code.append(f"  OffsetT *__restrict {inp['name'] + '_ptr'}, \n")
        else:
            # spm and vector x
            input_parameters_code.append(f"  ValueT *__restrict {inp['name'] + '_ptr'}, \n")
    ...
    for out in outputs:
        _name = out['name']
        input_parameters_code.append(f"  ValueT *__restrict {"output_y_" + _name + "_ptr"}, \n")
        ...
        if 'reducer' in str(out['target']):
            ...
            output_agent_tenosrs_code.append(f" OffsetT row_carry_out_{_name}[256]; \n")
            output_agent_tenosrs_code.append(f" TensorOutput_{_name}_T value_carry_out_{_name}[256]; \n")
    ...
    for inter in selector_register:
        ...
        if inter['selector'] == 1:
            ...
            selector_code.append(f"    OffsetT {'column_indices_' + _name} = {_selector_name + '_ptr'}[thread_coord.y]; \n")
            selector_code.append(f"    TensorInput_{_target}_T {_selector_name}({_target + '_ptr'} + {'column_indices_' + _name} * {_dim}); \n")
        else:
            # spm loading
...
def aggregator_gen(aggregator_operations):
    ...
    if aggregator_operations != []:
        ...
        aggregator_diagonal_code_search.append(f"    int2 thread_coord_end; \n")
        aggregator_diagonal_code_search.append(f"    thread_coord.y = tid * items_per_thread; \n")
        aggregator_diagonal_code_search.append(f"    thread_coord_end.y = std::min(tid * items_per_thread + items_per_thread, num_nonzeros); \n")

    for op in aggregator_operations:
        _dim = get_dim_length(op['shape'])
        _name = op['name']
        aggregator_partial_carry_fixup_code.append(f"    ApplyCarryOutFixup<ValueT, OffsetT, {_dim}>(num_threads, {_name}_running_carry_out, output_y_{_name}_ptr); \n")
        aggregator_partial_carry_fixup_code.append(f"    delete[] {_name}_running_carry_out; \n")
        aggregator_partial_forloop_code.append(f"    {_name}_running_carry_out[tid] += {op['args'][0]}; \n")
        aggregator_tenosrs_carry_out_code.append(f"    TensorOutput_{_name}_T* {_name}_running_carry_out=new TensorOutput_{_name}_T[num_threads]; \n")

    # # debug
    # debug_print(aggregator_partial_carry_fixup_code, "aggregator_partial_carry_fixup_code")
    return aggregator_partial_carry_fixup_code, aggregator_partial_forloop_code, aggregator_diagonal_code_search, aggregator_tenosrs_carry_out_code
...
def reducer_gen(reducer_operations):
    ...
    for op in reducer_operations:
        ...
        reducer_consume_init_code.append(f"   TensorOutput_{_name}_T reducer_{_name}_running_total; \n")
        reducer_consume_forloop_code.append(f"   reducer_{_name}_running_total += {_param}; \n")
        reducer_consume_forloop_add_code.append(f"   for (int i = 0; i < {_dim}; i++) \n")
        reducer_consume_forloop_add_code.append(f"   {{ \n")
        reducer_consume_forloop_add_code.append(f"     output_y_{_name}_ptr[thread_coord.x * {_dim} + i] = reducer_{_name}_running_total.values[i]; \n")
        reducer_consume_forloop_add_code.append(f"   }} \n")
        reducer_partial_init_code.append(f"   TensorOutput_{_name}_T {_name}_running_total_partial; \n")
        reducer_partial_forloop_code.append(f"   {_name}_running_total_partial += {_param}; \n")
        reducer_partial_carry_code.append(f"   row_carry_out_{_name}[tid] = thread_coord_end.x; \n")
        reducer_partial_carry_code.append(f"   value_carry_out_{_name}[tid] = {_name}_running_total_partial; \n")
