...
class System_calculation():
    ...
    def forward(self):
        ...
        self.output_y_agg_1 = torch.sum(self.output_y_map_1, dim=0)
        self.output_y_agg_2 = torch.sum(self.output_y_map_2, dim=0)
...
def system_test(
    ...
    output_y_agg_1 = torch.zeros(
        dim_output_map1,
        1,
        device=device,
        dtype=dtype)  # (M, D)
    output_y_agg_2 = torch.zeros(2, 3, device=device, dtype=dtype)  # (M, D)
