...
def analyze_tensor_distribution(compiled_module):
    ...
    for node in graph.nodes:
        ...
        #         print(f"meta[{k!r}] = {v!r}")
        #         # If v is an OrderedDict, print its items in detail
        #         if isinstance(v, dict) and v.__class__.__name__ == "OrderedDict":
        #             print(f"  meta[{k!r}] is an OrderedDict with items:")
        #             for subk, subv in v.items():
        #                 print(f"    {subk!r}: {subv!r}")
        # else:
        #     print("meta in node (not a dict):", node.meta)
...
#         # Outputs
#         self.output_y_map_1 = esr.Tensor(output_y_map_1, mode = 'partition')  # (N, 2)
#         self.output_y_map_2 = esr.Tensor(output_y_map_2, mode = 'partition')  # (N, 6)
#         self.output_y_reducer_1 = esr.Tensor(output_y_reducer_1, mode = 'partition')  # (M, 2)
#         self.output_y_reducer_2 = esr.Tensor(output_y_reducer_2, mode = 'partition')  # (M, 6)
...
#         self.output_y_reducer_1[:] = self.reducer_1(self.output_y_map_1)  # (M, 2)
#         self.output_y_reducer_2[:] = self.reducer_2(self.output_y_map_2)  # (M, 2, 3) (M, 6)
...
class System(esr.Module):
    ...
    def forward(self):
        r_1 = self.selector_1(self.vector_x)  # (N, 2)
        self.output_y_reducer_1[:] = self.reducer_1(r_1 * self.spm_1)  # (M, 2)
...
def system_test(
    ...
    # data settings
    dim_x = 2
    spm_1 = torch.rand(num_nnz, 2, 1, device=device, dtype=dtype)  # (N, 2, 1)
    spm_2 = torch.rand(num_nnz, 2, 3, device=device, dtype=dtype)  # (N, 2, 3)
    vector_x = torch.rand(
        num_cols,
        dim_x, 1,
        device=device,
    ...
    # Generate COO row indices - each element specifies which row the non-zero belongs to
    ...
    # model = System(
    #     spm_1, spm_2, vector_x, col_indices_1, col_indices_2, row_indices,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Run system test with configurable matrix dimensions')
