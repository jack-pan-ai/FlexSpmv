...
struct ReduceTensorByKeyOp
{
        ...
        const TensorT &second)      ///< Second partial reduction
    {
        ...
        if (first.key == second.key)
        {
            ...
            {
                retval.value[i] = first.value[i] + retval.value[i];
            }
        }
...
struct AgentFlexSpmv
{
    ...
    struct _TempStorage
    {
        ...
        union Aliasable
        {
            ...
            // the merged items include: row_end_offset, nonzero, value_vector_x (dimension will vary)
    ...
    SpmValueIteratorT               wd_sparse_matrix_A;   ///< Wrapped pointer to sparse matrix A
    ...
    :
        ...
        wd_sparse_matrix_A(spmv_params.d_sparse_matrix_A),
        ...
        Int2Type<false> is_direct_load)     ///< Marker type indicating whether to load nonzeros directly during path-discovery or beforehand in batch
    {
        ...
        for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM)
        {
            if (tile_nonzero_indices[thread_current_coord.y] < row_end_offset)
            {
                ...
                for (int i = 0; i < DIM_OUTPUT_VECTOR_Y; i++)
                {
                    scan_segment[ITEM].value[i] = nonzero[i];
        ...
        for (int i = 0; i < DIM_OUTPUT_VECTOR_Y; i++)
        {
            scan_item.value[i] = running_total[i];
        }
        ...
        if (tile_num_rows > 0)
        {
            ...
            if (scan_item.key != scan_segment[0].key)
            {
                ...
                for (int i = 0; i < DIM_OUTPUT_VECTOR_Y; i++)
                {
                    s_partials[scan_item.key * DIM_OUTPUT_VECTOR_Y + i] = scan_item.value[i];
                }
            }
            else
            {
                ...
                for (int i = 0; i < DIM_OUTPUT_VECTOR_Y; i++)
                {
                    scan_segment[0].value[i] += scan_item.value[i];
                }
            }
            ...
            for (int ITEM = 1; ITEM < ITEMS_PER_THREAD; ++ITEM)
            {
                if (scan_segment[ITEM - 1].key != scan_segment[ITEM].key)
                {
                    ...
                    for (int i = 0; i < DIM_OUTPUT_VECTOR_Y; i++)
                    {
                        s_partials[scan_segment[ITEM - 1].key * DIM_OUTPUT_VECTOR_Y + i] = scan_segment[ITEM - 1].value[i];
                    }
                }
                else
                {
                    ...
                    for (int i = 0; i < DIM_OUTPUT_VECTOR_Y; i++)
                    {
                        scan_segment[ITEM].value[i] += scan_segment[ITEM - 1].value[i];
                    }
                }
            }
        ...
        int                             num_merge_tiles)            ///< [in] Total number of merge tiles
    {
        ...
        if (threadIdx.x == 0)
        {
            if (HAS_ALPHA)
            {
                ...
                tile_carry.value = tile_carry.value * spmv_params.alpha;
            }
...
struct AgentSegmentFixup
{
    ...
    // Value type
    typedef typename TensorKeyPairT::ValueType ValueT;
        ...
        Int2Type<true>      use_atomic_fixup)   ///< Marker whether to use atomicAdd (instead of reduce-by-key)
    {
        ...
        for (int ITEM = 1; ITEM < ITEMS_PER_THREAD; ++ITEM)
        {
            ...
            if (pairs[ITEM].key != pairs[ITEM - 1].key){
                ...
                for (int i = 0; i < DIM_OUTPUT_VECTOR_Y; i++)
                {
                    atomicAdd(d_scatter + i, pairs[ITEM - 1].value[i]);
                }
            }
            else
        ...
        if ((!IS_LAST_TILE) || (pairs[ITEMS_PER_THREAD - 1].key >= 0))
        {
            ...
            for (int i = 0; i < DIM_OUTPUT_VECTOR_Y; i++)
            {
                atomicAdd(d_scatter + i, pairs[ITEMS_PER_THREAD - 1].value[i]);
            }
        }
    }
