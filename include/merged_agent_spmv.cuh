...
namespace merged
{
  ...
  typedef Tensor<ValueT, 2> TensorInput_y_add_1_T; 
  typedef Tensor<ValueT, 6> TensorInput_spm_2_T; 
  typedef Tensor<ValueT, 6> TensorInput_y_add_2_T; 
  ...
  typedef Tensor<ValueT, 2> TensorOutput_setitem_T; 
  typedef Tensor<ValueT, 6> TensorOutput_setitem_1_T; 
  ...
  VectorValueIteratorT y_add_1_ptr; 
  VectorValueIteratorT spm_2_ptr; 
  VectorValueIteratorT y_add_2_ptr; 
  ...
  ColumnIndicesIteratorT selector_2_ptr; 
    ...
    y_add_1_ptr(spmv_params.y_add_1_ptr), 
    spm_2_ptr(spmv_params.spm_2_ptr), 
    y_add_2_ptr(spmv_params.y_add_2_ptr), 
...
#pragma unroll
    ...
    VectorValueIteratorT y_add_1_ptr_current = y_add_1_ptr +                     (tile_start_coord.y + nonzero_idx) * 2; 
    TensorInput_y_add_1_T y_add_1(y_add_1_ptr_current); 
    ...
    VectorValueIteratorT y_add_2_ptr_current = y_add_2_ptr +                     (tile_start_coord.y + nonzero_idx) * 6; 
    TensorInput_y_add_2_T y_add_2(y_add_2_ptr_current); 
  ...
  for (int i = 0; i < 2; i++) 
  { 
    spmv_params.y_add_1_ptr[                (tile_start_coord.y + nonzero_idx) * 2 + i] =                     add_1.values[i]; 
  } 
  #pragma unroll 
  for (int i = 0; i < 2; i++) 
  { 
    y_add_1.values[i] = add_1.values[i];
  } 
  #pragma unroll 
  for (int i = 0; i < 6; i++) 
  { 
    spmv_params.y_add_2_ptr[                (tile_start_coord.y + nonzero_idx) * 6 + i] =                     add_3.values[i]; 
  } 
  #pragma unroll 
  for (int i = 0; i < 6; i++) 
  { 
    y_add_2.values[i] = add_3.values[i];
  } 
    temp_storage.smem_reducer_1.s_tile_value_reducer[nonzero_idx] =                     add_1; 
