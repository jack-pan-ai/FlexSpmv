...
namespace merged
{
  ...
  typedef Tensor<ValueT, 2> TensorOutput_sum_1_T; 
  typedef BlockReduce< 
            TensorOutput_sum_1_T, 
            BLOCK_THREADS, 
            BLOCK_REDUCE_WARP_REDUCTIONS> 
            BlockReduce_sum_1_T; 
  // Tensor type and block reducefor output 
  typedef Tensor<ValueT, 6> TensorOutput_sum_2_T; 
  typedef BlockReduce< 
        ...
        struct _TempStorage
        {
            ...
            // smem for intermediate results and scan
                           typename BlockReduce_sum_1_T::TempStorage smem_sum_1; 
...
#pragma unroll
  ...
  { 
    spmv_params.output_y_add_1_ptr[(tile_start_coord.y + nonzero_idx) * 6 + i] = add_1.values[i]; 
            ...
            // [code generation]
               // blockReduce 
   TensorOutput_sum_1_T sum_1_result = BlockReduce_sum_1_T(temp_storage.smem_sum_1).Sum(sum_1); 
   if (threadIdx.x == 0) 
   { 
     #pragma unroll 
     for (int i = 0; i < 2; i++) 
     { 
       atomicAdd(&spmv_params.output_y_sum_1_ptr[i], sum_1_result.values[i]); 
     } 
   } 
   // blockReduce 
